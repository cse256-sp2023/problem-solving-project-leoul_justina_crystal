      ...
      for (let ace_type in user_perms) {
        ...
        for (let perm in user_perms[ace_type]) {
          ...
          if (user_perms[ace_type][perm].inherited) {
            ...
            file_obj.acl.push(make_ace(user, perm, ace_type === "allow"));
          }
        }
      }
    }
...
function renderPermissionStatusUI() {
  var permissionsStateDiv = $("<div>").attr("id", "permissionsStateDiv");

  let $reloadButton = $("<button>");
  $reloadButton.text("Reload Issue Status");
  $reloadButton.on("click", function () {
    permissionsStateDiv.empty();
    $(permissionsStateDiv).append($reloadButton);
    populatePermissionsStateDiv();
  });

  $(permissionsStateDiv).append($reloadButton);
  $("#wrapper").append(permissionsStateDiv);
  populatePermissionsStateDiv();
}

// This function validates the current permissions state against the correct solution and creates a div that will show any discrepancies.
function populatePermissionsStateDiv() {
  // Get the current scenario and its correct solution state.
  let scenario = $("#scenario_context").data("tag");
  let scenario_solution_state = new Set(
    correct_scenario_solutions[scenario].split(";")
  );
  ...
  // Initialize empty arrays to store discrepancies in allowed and denied permissions.
  let missing_allowed = [];
  let should_not_be_allowed = [];

  // Check for permissions that should be allowed but are currently denied.
  for (let correct_allowed of scenario_solution_state) {
    if (!current_state.has(correct_allowed)) {
      missing_allowed.push(correct_allowed);
    }
  ...
  // Check for permissions that should be denied but are currently allowed.
  for (let current_allowed of current_state) {
    if (!scenario_solution_state.has(current_allowed)) {
      should_not_be_allowed.push(current_allowed);
    }
  ...
  let permissionsStateDiv = $("#permissionsStateDiv");
  console.log(permissionsStateDiv);
  // Time to render conditionally
  if (missing_allowed.length == 0 && should_not_be_allowed.length == 0) {
    permissionsStateDiv.append(
      "No need for action! All permissions are currently correct."
    );
  } else {
    permissionsStateDiv.append(
      "<p> Warning! Permission issues detected.\n </p>"
    );
    if (should_not_be_allowed.length > 0) {
      permissionsStateDiv.append(
        "<p> The following permissions are currently allowed, but should be denied:\n </p>"
      );

      var table = $("<table>").addClass("table");
      // Add border to table itself
      table.css("border", "1px solid black");

      permissionsStateDiv.append(table);
      for (let p of should_not_be_allowed) {
        let ids = p.split(":");
        let filepath = id_to_filepath[parseInt(ids[0])];
        let username = id_to_username[parseInt(ids[1])];
        ...
        //   $("<p>").text(`Specific Permission: ${perm}\n`),
        //   $("<p>").text(`Filepath: ${filepath}\n`),
        //   $("<br>"),
        //   $("<br>")
        // );
        var row = $("<tr>");
      ...
      table.find("td, tr").css("border", "0.5px solid gray");
    }
    if (missing_allowed.length > 0) {
      permissionsStateDiv.append(
        "<p> The following permissions are currently denied, but should be allowed:\n </p>"
      );
      var table = $("<table>").addClass("table");
      // Add border to table itself
      table.css("border", "0.5px solid gray");
      ...
      // Add border to table cells
      table.css("border-collapse", "collapse");
      table.find("td, tr").css("border", "1px solid black");
    }
  }

  //   console.log(should_not_be_allowed);
  //   for (let t of should_not_be_allowed) {
  //     let ids = t.split(":");
  //     let filepath = id_to_filepath[parseInt(ids[0])];
  //     let username = id_to_username[parseInt(ids[1])];
  //     let perm = id_to_permission[parseInt(ids[2])];
  ...
  //         "The following permissions are currently allowed, but should be denied in a correct solution:"
  //       );
  //       // Log each permission that should be denied but is currently allowed.
  //       for (let p of should_not_be_allowed) {
  //         let ids = p.split(":");
  //         let filepath = id_to_filepath[parseInt(ids[0])];
  //         let username = id_to_username[parseInt(ids[1])];
  ...
  //         "The following permissions are currently denied, but should be allowed in a correct solution:"
  //       );
  //       // Log each permission that should be allowed but is currently denied.
  //       for (let p of missing_allowed) {
  //         let ids = p.split(":");
  //         let filepath = id_to_filepath[parseInt(ids[0])];
  //         let username = id_to_username[parseInt(ids[1])];
  ...
  //     );
  //   } else {
  //     // If there are no discrepancies, log the current user data.
  //     console.log(JSON.stringify(userData));
  //   }
}
